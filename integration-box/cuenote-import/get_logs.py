...
def main():
    ...
    for expid in expids['data']:
        for export in cuenote.call_api('getExportStatus', {'expid': str(expid[0])}).iter('export'):
            for item in export:
                ...
                if item.tag == 'log_clickcount':
                    df.columns = ['clicked_at', 'clicked_url', 'email_address', 'click_count', 'member_id', 'delivid']
                    df['clicked_at_c'] = pandas.to_datetime(df['clicked_at'])

                elif item.tag == 'log_deliv':
                    df.columns = ['email_address_id', 'email_address', 'status_updated_at', 'status_loc', 'status',
                                  'mx_host_name', 'connection_ip_port', 'smtp_status_updated_at', 'smtp_status_loc',
                                  'smtp_status', 'smtp_response', 'bounce_received_at', 'bounce_type', 'bounce_summary',
                                  'bounce_content', 'bounce_address', 'bounce_log_id', 'unreachable_at', 'all_retries',
                                  'first_retry', 'last_retry', 'retry_count', 'last_retry_status_loc', 'last_retry_status',
                                  'last_retry_response', 'member_id', 'delivid']
                    df['status_updated_at'] = pandas.to_datetime(df['status_updated_at'])
                    df['smtp_status_updated_at'] = pandas.to_datetime(df['smtp_status_updated_at'])
