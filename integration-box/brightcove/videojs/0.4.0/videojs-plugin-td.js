(function () {
  ...
  registerPlugin('td', function(options) {
    ...
    TdLogger = function(options) {
      ...
      this.td.set(this.table, {'bc_session_id': player.bcAnalytics.client.defaultParams_.session});
    ...
    loaded = function() {
      tdLogger.td.set(tdLogger.table, {'bc_video_id': this.mediainfo.id});
      tdLogger.td.set(tdLogger.table, {'bc_duration': this.mediainfo.duration});
      tdLogger.td.set(tdLogger.table, {'bc_video_name': this.mediainfo.name});
      if(typeof percentsAlreadyTracked[this.mediainfo.id] === 'undefined') {
        percentsAlreadyTracked[this.mediainfo.id] = [];
      }
    ...
    percentOfView =function() {
      var percentArr = percentsAlreadyTracked[this.mediainfo.id] || (percentsAlreadyTracked[this.mediainfo.id] = [], percentsAlreadyTracked[this.mediainfo.id]);
      var currentTime, duration, percent, percentOfViewed, _i;
        ...
        for (percent = _i = 0; _i <= 100; percent = _i += interval) {
          if (percentOfViewed >= percent && _indexOf.call(percentArr, percent) < 0) {
