...
## What data will you need build this model?
...
In the template code, we're using two examples - `pageviews_table` and `formfills_table`. In order to model **Multi-Touch Attribution** (MTA), you need to define which touchpoints count as conversion events in the data.
...
The final output of the first data pre-processing step above should be a single `UNION` table, where each row is a timestamped touchpoint that includes a unique user identifier, the channel/source/campaign associated with that event and whether that event was a conversion or not.  This is often done by using SQL code to union multiple user-activity tables together, which could be a combination of both digital and physical touchpoints, depending on what marketing campaigns the client is actively running, and whether that data is available in the CDP. 

See an example table schema below:
...
This table is created by the [`new_union.sql`](../queries/new_union.sql) query in the example workflow. Note that this query needs to be modified and customized to fit your data, use cases and business rules for interpreting `utm_source`, medium, and channel parameters and defining conversion events.

## How to configure the Python model after running the first few SQL functions in workflow to create the final `UNION` table with `ab_rnd` that will be used by the LSTM Python model?

Before you build a model, configuration parameters must be properly defined in [`td_mta/config.py`](../td_mta/config.py). See more details below:
...
It is also important to copy your Master API Key from your TD account and create a `secret` in the workflow called `td_api_key`, where you can paste the API Key to keep it safe. That secret is then interpreted as an environmental variable `TD_API_KEY`, which is needed by the Python code to read and write data to TD.

## What to do if Workflow returns ERROR because of the docker image function reaching it's max limit capacity?

Depending on a tier your contract determines, the `py>` tasks have a memory limit of 8/30GB and one/four virtual CPUs (no GPUs), which might result in an error if we attempt to run the model on a very large volume of data. 

In our experiment, the model ran a successfully with the larger tier on 20M rows of data and 2 Epochs, which took about 10-12hrs. However, when attempting on 30M rows of data and 5 epochs, the model tool over 24hrs to run, which caused TD to automatically kill the Workflow; TD has a 24hr limit per single process, so any model configuration that takes 24+ hrs of runtime, would need to be run outside of TD. 

Hence, note that on large volumes of data and running model with higher matrix dimensionality (e.g., Longer lookback periods, more distinct channels in the 'action column'), it will take about 5-6hrs per training Epoch and another 2-4 hours for TF record creation and random shuffling + Shapely Value calculations, so expect model to take 24+ hrs to complete. In those cases, we advise running only the SQL portion of the workflow first, and once the final union table with ab_rand attribute is created, download the python code and run locally or inside an EC2 instance. More instructions on how to do that can be found in [`td_mta/README.md`](../td_mta/README.md). 
...
1.  Run SQL code in TD workflow 
    - You need to comment out the `py>` part of the workflow and then run the workflow to only execute the initial SQL data transformation steps and output the final union table of touchpoints and conversions.
...
3.  Follow the steps shared earlier in the doc on how to configure the `config.py` file with the proper database, table names and hyperparameters before you execute the `main.py`. 
4.  In CLI, navigate to the `td_mta` project folder directory and first make sure you set your `TD_API_KEY` as an environ variable by using command bellow in:
...
```python3 -m td_mta.main``` - this will trigger the `main.py` file in the `td_mta` folder and you will be able to see the logs that the Python code is programmed to print in the CLI.
...
## What tables in TD does the model output and how to interpret the data in them?

**Merics_table** - the table will contain `loss` and `rmse` parameters and will contain a row for each epoch ran and columns for training and validation. The main thing to watch for is to notice if val_loss or RMSE stop decreasing and start getting bigger, which is when you might want to stop the model next time it runs. You also want to make sure that there is not a huge difference between training and validation metrics, because that might point to model overfitting. 
 
**Shapely_values_table** - this is the table we've all been waiting for! The number of rows would equal the number of lookback days that you chose in the `config.py` file. There will be a column called `days_before_conversion` where 0 indicates the effect of each channel on the conversion event when users are exposed to that channel less than 24hrs before that conversion event. Going up the days index, you will see how Shapley Values / attribution percentages change the farther the touchpoint is from the final conversion event. This table gives insights into how different channels perform throughout the customer journey to conversion and which ones are more effective as a `First-touch` vs. `Last-touch` in that journey. 
 
**Shapley_channel**  - the code outputs the aggregate Shapley values summed across all days of the lookback window in a separate table with the `_prefix`. These values can be considered as the total attribution percentage that each channel should be assigned towards conversions. 
