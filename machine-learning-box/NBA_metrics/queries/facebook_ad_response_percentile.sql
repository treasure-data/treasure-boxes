...
select A.${activity_table_join_key},sum((case
                          when ${utm_source_column} like '%facebook%' then 1
                          else 0
                          end)) as no_of_visit
...
T4 as (
  SELECT
   ${activity_table_join_key},
  CAST(FLOOR((1 - utm_rank / total_user) * 100) as INTEGER) as facebook_ad_response_percentile
  FROM T3
)
SELECT u.*,coalesce(facebook_ad_response_percentile,0) as facebook_ad_response_percentile
