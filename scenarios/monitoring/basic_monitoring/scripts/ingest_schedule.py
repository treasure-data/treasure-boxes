# https://api-docs.treasuredata.com/pages/td-api/tag/Schedules/
...
def get_all_schedule(url, headers):
  print(url)
...
def run(dest_db, dest_table, api_endpoint='api.treasuredata.com'):
  url = 'https://%s/v3/schedule/list' % api_endpoint
  headers = {'Authorization': 'TD1 %s' % os.environ['TD_API_KEY']}
  l = get_all_schedule(url, headers)
  if len(l) == 0:
