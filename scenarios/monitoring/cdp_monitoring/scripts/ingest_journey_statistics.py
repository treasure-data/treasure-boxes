...
def get_journey_statistics(base_url, headers, id):
    url = base_url + '/entities/journeys/' + str(id) + '/statistics'
    print(url)
...
def get_all_journey_statistics(base_url, headers, ids):
    l = []
    for i in ids:
        d = get_journey_statistics(base_url, headers, i)
        if d != None:
            l.append(d)
    return l
...
def run(session_unixtime, dest_db, dest_table, journey_ids, api_endpoint='api.treasuredata.com', cdp_api_endpoint='api-cdp.treasuredata.com'):
    print('ingest journey statitics')
    ...
    l = get_all_journey_statistics(base_url, headers, id_list)
    if len(l) == 0:
