...
(
    SELECT
        TD_TIME_PARSE(JSON_EXTRACT_SCALAR(exe, '$.createdAt'), '${td.timezone}') AS time
        ,TD_TIME_PARSE(JSON_EXTRACT_SCALAR(exe, '$.finishedAt'), '${td.timezone}') AS time_finished
    ...
    WHERE ps_id = '${ps_id}'
    AND TD_TIME_RANGE(TD_TIME_PARSE(JSON_EXTRACT_SCALAR(exe, '$.createdAt'), '${td.timezone}'), ${time_from}, ${time_to})
)
